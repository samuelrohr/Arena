...
public class FollowTarget : MonoBehaviour {
	...
	void LateUpdate () {
		...
		camAngle = Mathf.LerpAngle(camAngle, nextAngle, rotationDamping * Time.deltaTime);
	...
	void FixedUpdate(){
		...
		if(target.GetComponent<CarController>().currSpeed < -5){
			rotationVector = new Vector3(0, target.eulerAngles.y + 270, 0);
		}
		else{
			rotationVector = new Vector3(0, target.eulerAngles.y + 90, 0);
		}

		//Faz o efeito de zoom out quando o target anda
		camera.fieldOfView = defaultFOV + target.rigidbody.velocity.magnitude * zoomRatio;
	}
}
